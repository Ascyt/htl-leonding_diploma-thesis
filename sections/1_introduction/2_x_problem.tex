...
So-called \textit{KeTops} are used as operating devices to control machines of varying structure and complexity. These machines and their corresponding \textit{KeTops} differ in their configuration for each customer, which is why appropriate test scripts are required. These test scripts examine all aspects of the device from the installed operating system to software specific behaviors. 

The old \textit{Script Assembler}, which was used in production prior to our new implementation, is able to merge a number of test scripts together for this use. The hard-coded nature and resulting limited maintainability of this \textit{Script Assembler} is its primary flaw. Furthermore, all data concerning tests are saved and maintained in a TestRail database, which, although functional, heavily limits the expandability and independence of the testing environment.
