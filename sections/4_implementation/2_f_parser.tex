...
The Lark grammar, in the method header labeled as \verb|ebnf_grammar|, represents \textit{a list of rules and terminals that together define a language} as a string. While the Lark library uses its own syntax, it is based on \gls{ebnf}.\cite{py-module-lark-grammar}

In our project, the following string, which is a modified form of the \textit{final \gls{ebnf}} which shown in the previous part to abide by Lark's expected syntax, gets used:
...
A \verb|Tree| object represents a rule with a name, for example, the outermost rule of a parsed script being \verb|"start"|, alongside, if present, its children, which, in the example of \verb|"start"| are \verb|"line"|s. A \textit{child} is commonly a \verb|Tree| as well, however it may also be a \verb|Token|. 

A \verb|Token| is a \textit{string with meta-information}, which are chunks of the input text that \textit{haven't been discarded}. The \verb|Token| class inherits from Python's \verb|str|, which allows it to be treated as string.\cite{py-module-lark-api}
