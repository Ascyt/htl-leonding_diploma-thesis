...
Monarch basically just uses \gls{regex} to handle the tokenization. The step-by-step-process starts with the lexer, that reads the document text in order and tries  to match the defined regular expression patterns. When it finds a match, the matched text is assigned a specific token class. This can be a specific keyword or a comment or it could also just be a newly defined custom modifier. The editor then maps these token classes to the specific color values defined in the active theme.

A global list of \gls{regex} rules is sufficient for simple languages. But for the custom annotations there is a need for context-aware parsing. For example, the arguments that follow an \texttt{@VERSION} annotation should be highlighted, but if the same argument is written in a comment it should not.

Monarch's state machine solved this problem. The lexer starts in a default \texttt{@root} state, when a specific pattern is matched, the lexer jumps into a different, explicitly defined state, and different \gls{regex} rules apply in this new state.
...
The \gls{regex} for the semantic version argument might look complicated and hard to maintain, but it is the officially provided \gls{regex} by the organization behind semantic versions. 
...
To fix this ``state bleed'', the lexer had to recognize line breaks as termination triggers. Normally, Monarch works line-by-line and does not pass line break characters to the \gls{regex} engine. To change this, the \texttt{includeLF} property was enabled in the language configuration (see Listing~\ref{lst:monarch_config_lf}).
...
For example for the case, when a developer accidentally types a  \texttt{@BRACKETSTART argument}, the lexer first recognizes the annotation, jumps to the \texttt{endOfLine} state. There, it expects the end of the line. Instead, it encounters a space, and the word ``argument''. Because these characters do not match the \texttt{/\textbackslash n/} \gls{regex}, they fall through to the \texttt{/./} rule.
